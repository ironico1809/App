...
const HistorialPagos = () => {
  ...
  const preview = (p) => {
    ...
    w.document.write(`<pre style="font-family: ui-monospace, Menlo, Consolas; padding:16px;">${`Comprobante de pago\nID: ${p.id}\nFecha: ${p.fecha} ${p.hora}\nUnidad: ${p.unidad}\nConcepto: ${p.concepto}\nPeriodo: ${p.periodo}\nMétodo: ${p.metodo}\nMonto: Bs ${p.monto.toFixed(2)}`}</pre>`);
    w.document.close();
  };
        ...
        <div className="hp-container">
          ...
          <div className="hp-summary">
            <div className="hp-card">
              ...
              <div className="hp-amount">Bs {totalAprobado.toFixed(2)}</div>
          ...
          <div className="hp-table">
            ...
            {filtrados.map(p => (
              <div className="hp-row" key={p.id}>
                ...
                <div>{p.periodo}</div>
                <div>{p.metodo}</div>
                <div className="hp-amount-cell">{p.monto.toFixed(2)}</div>
                <div>{estadoBadge(p.estado)}</div>
                <div className="hp-actions">
                  {p.estado === 'Aprobada' ? (
                    <>
                      <Button className="small" onClick={() => download(p)}>Descargar</Button>
                      <Button className="outline small" onClick={() => preview(p)}>Vista previa</Button>
                    </>
                  ) : (
